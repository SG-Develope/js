...
async function addItem(title: string) {
  try {
    await axiosInstance.post<TodoListRes>(`/todolist`, { title });
    showList();
  } catch (err) {
    if (err instanceof AxiosError) {
...
async function toggleDone(no: number) {
  ...
  try {
    await axiosInstance.patch<TodoListRes>(`/todolist/${no}`, { done: isDone });
    showList();
  } catch (err) {
    if (err instanceof AxiosError) {
      console.log("등록 실패.", err.response?.data.message);
...
(document.querySelector(
  ".todoinput > input"
) as HTMLInputElement)!.addEventListener("keyup", handleKeyup);
showList();
